        ...
        <div class="flex flex-col h-full">
          ...
          >
            ...
            <p class="text-center">
              {{
                searchQuery
                  ? "Tidak ditemukan ruang kelas yang sesuai."
                  : "Belum ada ruang kelas yang diinputkan."
              }}
            </p>
          </div>
...
const filteredRuangKelasList = computed(() => {
  ...
  return ruangKelasList.value.filter(
    (ruang) =>
  ...
  try {
    ...
    if (editIndex.value !== null) {
      ...
      const response = await axios.put(
        `http://localhost:5000/ruangan/${ruangKelasKode}`,
        newRuangKelas,
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
const editRuangKelas = (index) => {
  ...
  selectedIndex.value = ruangKelasList.value.findIndex(
    (r) => r.ruangan_kode === ruang.ruangan_kode
  );
...
const confirmDelete = async () => {
  try {
    ...
    await axios.delete(
      `http://localhost:5000/ruangan/${ruangKelas.ruangan_kode}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
