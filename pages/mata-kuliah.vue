...
const submitMataKuliah = async () => {
  ...
  try {
    ...
    if (editIndex.value !== null) {
      ...
      await axios.patch(
        `http://localhost:5000/mata_kuliah/${mataKuliahKode}`,
        newMataKuliah,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      // Add New Mata Kuliah
      await axios.post("http://localhost:5000/mata_kuliah", newMataKuliah, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
const confirmDelete = async () => {
  try {
    ...
    await axios.delete(`http://localhost:5000/mata_kuliah/${mk.matkul_kode}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
